...
def register_user(user_json,name,password,age,phn,email,Address):
	user={
					...
					for j in range(len(content)):
						if content[j]["id"]==user_id:
							...
							if date not in content[j]["order_history"]:
								content[j]["order_history"][date]=[[content1[i]["name"],content1[i]["price"]]]
								print("your order-"+content1[i]["name"],content1[i]["price"])
							else:
								content[j]["order_history"][date].append([content1[i]["name"],content1[i]["price"]])
								print("your order-"+content1[i]["name"],content1[i]["price"])
				else:
...
def add_food(user_json,food_name,price,no_of_plates=1):
	food={
...
def read_food(file_json):
	...
	print("Menu")
	for i in range(len(content)):
		print(f"{content[i]['id']}-{content[i]['name']}-{content[i]['price']}.RS-{content[i]['no_of_plates']} plates")
...
#-------------Register-------------------
		...
		print(register_user("users.json",name,password,age,phone,email,address))
						...
						if content[i]["password"]==password:
							while True:
								...
								if val3==1:
									...
									foodid=list(map(int,input("Enter foodid separated by space").split()))
					...
					if content["password"]==password:
						while True:
							...
							if val3==1:
								...
								print(add_food("fooditem.json",foodname,price,quantity))
							elif val3==2:
								print()
